...
const OrdersRow = ({order, index, setOrderDelete}) => {
    return (
        <tr key={index}>
            ...
            <td>
                {!order.paid && <label onClick={() => setOrderDelete(order)} htmlhtmlFor="order-delete-modal" className="btn btn-xs bg-white"><FontAwesomeIcon className='text-xl' icon={faTrashCan} /></label>}
            </td>
        </tr>
    );
};
